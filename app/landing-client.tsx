    ...
    <div className="flex h-screen bg-[#120F17] overflow-hidden" style={{ width: "100vw" }}>
          ...
          <div data-parallax="0.06" style={{ willChange: "transform" }} className="flex items-center gap-8 px-10 py-20">
            <div className="flex-1 min-w-0 pr-4">
              ...
              <p className="text-white/50 text-base leading-relaxed max-w-[40ch] mb-5">
                win a match, the number goes up. lose, it goes down. no participation trophies, no
          ...
          <div data-parallax="0.06" style={{ willChange: "transform" }} className="flex flex-row-reverse items-center gap-8 px-10 py-20">
            <div className="flex-1 min-w-0 pl-4">
              ...
              <p className="text-white/50 text-base leading-relaxed max-w-[40ch] mb-6">
                skip the lobby. tap play, we find your opponent instantly. you&apos;re mid-prep,
                not mid-wait.
              </p>
              <div className="flex flex-wrap gap-2">
                <span className="text-[10px] font-mono text-[#118ab2] border border-[#118ab2]/25 bg-[#118ab2]/8 rounded-full px-3 py-1.5 tracking-wider">
                  VARC ×1 speed
                </span>
                <span className="text-[10px] font-mono text-[#ffd166] border border-[#ffd166]/25 bg-[#ffd166]/8 rounded-full px-3 py-1.5 tracking-wider">
                  DILR ×2 speed
                </span>
                <span className="text-[10px] font-mono text-[#06d6a0] border border-[#06d6a0]/25 bg-[#06d6a0]/8 rounded-full px-3 py-1.5 tracking-wider">
                  QUANT ×2 speed
                </span>
              </div>
            </div>
          ...
          <div data-parallax="0.06" style={{ willChange: "transform" }} className="flex flex-row-reverse items-center gap-8 px-10 py-20">
            <div className="flex-1 min-w-0 pl-4">
              ...
              <p className="text-white/50 text-base leading-relaxed max-w-[40ch]">
                send a link. they accept or they&apos;re cooked. rated or unrated, 15 min expiry.
                no excuses, no drama.
              </p>
            </div>
          ...
          <footer className="px-10 py-12 border-t border-[#9f84bd]/10">
            <div className="flex items-center justify-between flex-wrap gap-4">
              ...
              <div className="flex items-center gap-6">
                <a href="/privacy" className="text-white/35 hover:text-white/60 text-xs transition-colors">
                  Privacy Policy
                </a>
            ...
            <p className="text-white/20 text-xs mt-6 font-mono">
              © 2026 Ninjatest. Built for the CAT grind.
            </p>
          </footer>
      ...
      >
        ...
        >
          <button
            onClick={handleBack}
            className="absolute top-6 left-6 text-[#120F17]/60 hover:text-[#120F17] text-sm font-medium transition-colors"
          >
            ← Back
          </button>
...
function EloRing() {
  ...
  const pct = animated ? TARGET / MAX : 0;
  ...
  return (
    <div ref={ref} className="flex flex-col items-center gap-4">
      <svg width="200" height="200" viewBox="0 0 200 200">
        ...
        <circle
          cx="100" cy="100" r={r}
          fill="none" stroke="#06d6a0" strokeWidth="10"
          strokeLinecap="round"
      ...
      <div className="flex items-center gap-3">
        <span className="text-[#06d6a0] font-bold text-sm font-mono">+24</span>
        <span className="text-white/25 text-xs">from last match</span>
      </div>
...
function MatchAnimation() {
  ...
  useEffect(() => {
    const interval = setInterval(() => setFound(f => !f), 2600);
  ...
  return (
    <div className="flex flex-col items-center gap-5 p-4 w-full">
      <div className="h-6 flex items-center justify-center">
        {found ? (
          <span className="bg-[#06d6a0] text-[#120F17] text-[9px] font-black tracking-[0.2em] uppercase px-3 py-1 rounded-full">
            match found
          </span>
        ) : (
          <span className="text-[#9f84bd]/40 text-[9px] font-mono tracking-[0.2em] uppercase">
            searching...
          </span>
        )}
      </div>
      ...
      <div className="flex items-center gap-5 justify-center w-full">
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          <span className="text-white/35 text-[9px] font-mono uppercase tracking-wider">
            {found ? "opponent" : "waiting"}
          </span>
        </div>
      </div>
...
function SpeedTimer() {
  ...
  useEffect(() => {
    if (!active) return;
    const interval = setInterval(() => setSecs(s => s <= 0 ? 90 : s - 1), 110);
    return () => clearInterval(interval);
  }, [active]);

  const pct = secs / 90;
  const pts = Math.round(100 * pct);
  const r = 52;
  const circ = 2 * Math.PI * r;
    ...
    <div ref={ref} className="flex flex-col items-center gap-5">
      <div className="relative">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className="text-white font-black text-xl leading-none" style={{ fontFamily: "var(--font-geist-mono, monospace)" }}>
            {secs}s
          </span>
  ...
  return (
    <div className="bg-[#120F17]/55 border border-[#9f84bd]/15 rounded-2xl p-5 backdrop-blur-sm">
      <div className="flex items-center gap-3 mb-4">
        <div className="w-9 h-9 rounded-full bg-[#9f84bd]/12 flex items-center justify-center text-lg shrink-0">
          🥷
        </div>
      ...
      <div className="flex items-center gap-2 bg-[#120F17]/80 rounded-lg p-3 mb-4 border border-[#9f84bd]/8">
        ...
        <button
          ...
          className={`text-[10px] font-bold px-2.5 py-1 rounded-md transition-all shrink-0 ${
            copied ? "bg-[#06d6a0] text-[#120F17]" : "bg-[#9f84bd]/12 text-[#9f84bd]/70"
          }`}
        >
          {copied ? "copied ✓" : "copy"}
        </button>
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-[10px] font-mono text-[#9f84bd]/35 bg-[#9f84bd]/5 border border-[#9f84bd]/8 rounded-lg p-3 mb-4">
        ...
        <span className="text-[#ef476f]/60">⚔ rated</span>
        <span>expires 14:32</span>
      </div>

      <button className="w-full bg-[#06d6a0] text-[#120F17] font-bold text-sm rounded-xl py-2.5 hover:bg-[#06d6a0]/90 transition-colors">
...
function LogoMarquee() {
  const items = [
    "VARC", "DILR", "QUANT", "ELO RATED", "REALTIME 1v1",
    "SPEED SCORING", "FRIEND CHALLENGES", "SERVER AUTHORITATIVE",
      ...
      <div className="flex animate-marquee" style={{ width: "max-content" }}>
        {[...items, ...items].map((item, i) => (
          <span key={i} className="shrink-0 px-6 text-[#9f84bd]/22 text-[10px] font-mono tracking-[0.3em] uppercase">
            {item}
            <span className="ml-5 text-[#9f84bd]/12">✦</span>
          </span>
        ))}
      </div>
    </div>
