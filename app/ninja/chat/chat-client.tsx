...
export default function ChatClient() {
  ...
  const send = async (raw: string) => {
    ...
    const reqMode = mode === "buddy" ? "socratic" : undefined;
    setInput("");
    setBusy(true);
    ...
    <div className="fixed inset-0 flex bg-[#120F17] md:pl-20">
      ...
      <div className="flex flex-1 flex-col min-w-0">
        <div className="flex items-center gap-2 border-b border-[#222222] px-4 py-3">
          ...
          <span className="ml-auto text-[11px] text-[#4a8fa8]">
            {mode === "buddy" ? "Socratic — I guide, you solve" : "Grounded in your real stats"}
          </span>
        </div>
...
function Hero({ mode, onPick }: { mode: "coach" | "buddy"; onPick: (q: string) => void }) {
  return (
    <div className="flex h-full flex-col items-center justify-center gap-5 px-6 text-center">
      <NinjaLogo color="#06d6a0" className="h-12 w-12" />
      <div>
        <h1 className="text-white text-xl font-semibold">Ask Ninja anything</h1>
        <p className="mt-1 text-[#7ab5cc] text-sm max-w-md">
          {mode === "buddy"
            ? "I'll guide you through your weak spots step by step."
            : "Grounded in your real stats — sections, margins, streaks, and recent mistakes."}
        </p>
      </div>
