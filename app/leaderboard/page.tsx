      ...
      <main className="max-w-2xl mx-auto px-4 py-6">
          ...
          {entries.map((entry) => {
            ...
            return (
              <Link key={entry.username} href={`/profile/${entry.username}`}>
                <div className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-colors hover:bg-[#1c2d38] ${
                  isMe ? "bg-[#00ed64]/5 border border-[#00ed64]/20" : ""
                }`}>
                  {/* Rank */}
                  <div className="w-8 text-center shrink-0">
                    {entry.rank <= 3 ? (
                      ...
                      <span className="text-[#5c6c7a] text-sm font-mono">#{entry.rank}</span>
                    )}
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[#5c6c7a] text-xs">{winRate} win rate</p>
                  </div>

                  {/* ELO */}
                  <div className="text-right shrink-0">
                    <p className="text-[#00ed64] font-bold">{entry.elo}</p>
                    <p className="text-[#3d4f5b] text-xs">{entry.wins}W {entry.losses}L</p>
                  </div>
                </div>
              </Link>
            );
        ...
        {entries.length === 0 && (
          <div className="text-center py-16">
            <p className="text-[#5c6c7a]">No players yet. Be the first!</p>
          </div>
        )}
      </main>
    </div>
