...
- `passages` — **currently empty (0 rows)**, so every VARC/DILR question is standalone, the picker's passage-group branch never fires, and `section_config.reading_ms` can never apply (it keys on `passage_id is not null`). A content gap, not a code bug.
