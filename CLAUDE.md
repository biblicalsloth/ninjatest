...
- **Admin console removed 2026-07-19** — `/admin`, `/admin/waitlist`, and the admin-only API routes (`/api/ninja/{generate,audit,distractors,extract,anticheat}`) are deleted from the app; `ADMIN_ENABLED` middleware branching is gone. The admin RPCs and `profiles.is_admin` still exist in the DB (dropping them needs a prod migration); question-bank work goes through the `scripts/` ingest path or SQL.
...
- `POST /api/ninja/*` — the AI layer (5 user-facing handlers: ask, coach, solve, daily, debrief). Same rules: no `proxy.ts` coverage, so **every one does its own auth, admin gate, and rate limiting inline**. See Ninja AI below.
...
Live = the notion `join_queue` uses: caller in a match with `status in ('active','pending')`, read through `matches`'s participants-only RLS, no new RPC. **Fail-closed** — a read error blocks the call, matching the metered-LLM limiters. Guarded by `scripts/ninja-guard-test.sql` sections 6 and 8; add any new user-facing route to the gate. (The admin-only routes that were deliberately ungated are deleted as of 2026-07-19.)
...
| ~~admin console~~ | — | **removed 2026-07-19** — admin code deleted from the app; retire the Vercel project + `ADMIN_ENABLED` var | — | — |
...
| `ADMIN_ENABLED` | **nothing reads it** (since 2026-07-19) | Was the admin-console deployment switch; the admin feature is deleted. Remove from `.env.local` and the retired admin Vercel project. (Historical: `FOO= npm run dev` does NOT unset — `@next/env` lets `.env.local` win; use a non-empty sentinel. Still true for `NEXT_PUBLIC_APP_MODE`.) |
