-- accept_challenge: adaptive fill (mixed) + 9-question guard (all modes).
--
-- Same truncation bug as try_match_internal (20260718000000) and match_with_bot
-- (20260718040000): accept_challenge built q_ids and inserted the match WITHOUT
-- ever checking it holds 9. Three ways to under-fill:
--   · mixed (section_mode null): VARC(3)||DILR(3)||QUANT(3) — a section with < 3
--     active questions returns NULL -> '{}', so the match truncates to 6 or 3.
--   · single-section else branch: `limit 9` over one section returns < 9 when
--     that section has fewer than 9 active questions.
--   · QUANT branch: pick_quant_question_ids(target, 9) can return < 9 on a thin
--     bank.
-- Any of these makes current_index run past question_ids and get_match_question
-- deref question_ids[index+1] = NULL — the game glitches mid-match.
--
-- Fix: the mixed branch now uses the same adaptive fill as try_match_internal /
-- match_with_bot (an empty VARC/DILR rolls its 3 slots into QUANT), and a single
-- <> 9 guard after all branches refuses to create a short match — the host and
-- guest get a clean error instead of a corrupt match. Section-mode challenges
-- can't roll into QUANT (the user picked the section), so they simply raise when
-- the chosen section is too thin. Happy path (full bank) is unchanged.
--
-- Starts from the latest def (20260716220817_prefer_unseen_questions.sql), disc
-- #1. Signature unchanged (text) so no overload fork (disc #6). search_path
-- pinned inline, grants re-applied (disc #2).
